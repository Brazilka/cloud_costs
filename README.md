...
- `step1_prepare_data.py` is the local learning version. It reads CSV files.
...
- `COSTS_TABLE`: BigQuery table id, for example `my-project.billing.costs`
- `FLOW_OWNERSHIP_TABLE`: BigQuery table id, for example `my-project.billing.flow_ownership`
- `PROJECT_OWNERSHIP_TABLE`: BigQuery table id, for example `my-project.billing.project_ownership`
...
gcloud run deploy cost-anomaly-detector \
  ...
  --set-env-vars COSTS_TABLE="PROJECT.DATASET.costs" \
  --set-env-vars FLOW_OWNERSHIP_TABLE="PROJECT.DATASET.flow_ownership" \
  --set-env-vars PROJECT_OWNERSHIP_TABLE="PROJECT.DATASET.project_ownership" \
